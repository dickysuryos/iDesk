...
String incomingCallNotificationUUID;
...
void onNotificationArrived(const ArduinoNotification *notification, const Notification *rawNotificationData)
{
    ...
    if (notification->category == CategoryIDSocial)
    {
        // If this is an incoming call, store it so that we can later send a user action.
        incomingCallNotificationUUID = notification->title;
        Serial.println("--- INCOMING CALL: PRESS A TO ACCEPT, C TO REJECT ---");
    }
    else
    {
        incomingCallNotificationUUID = ""; // Make invalid - no incoming call
    }
}
...
void loop()
{
    if (incomingCallNotificationUUID != "")
    {
        Serial.println("get notif");
